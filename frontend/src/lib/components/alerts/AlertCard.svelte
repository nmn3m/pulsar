...
<div
  class="bg-white p-4 rounded-lg shadow hover:shadow-md transition-shadow cursor-pointer border-l-4 {priorityColors[
    alert.priority
  ]}"
  ...
  <div class="flex items-start justify-between">
    ...
    <div class="flex gap-2 ml-4" on:click|stopPropagation on:keydown|stopPropagation role="none">
      {#if alert.status === 'open'}
        <Button variant="primary" on:click={() => onAcknowledge(alert.id)}>Acknowledge</Button>
      {/if}
      {#if alert.status !== 'closed'}
        <Button variant="secondary" on:click={() => onClose(alert.id)}>Close</Button>
      {/if}
    </div>
  </div>
</div>
