...
class APIClient {
  ...
  async listTeams(page = 1, pageSize = 20): Promise<{ teams: Team[] }> {
    return this.request<{ teams: Team[] }>(`/api/v1/teams?page=${page}&page_size=${pageSize}`);
  }
  ...
  async removeParticipant(scheduleId: string, rotationId: string, userId: string): Promise<void> {
    await this.request(
  ...
  async getEscalationRule(policyId: string, ruleId: string): Promise<EscalationRule> {
    return this.request<EscalationRule>(`/api/v1/escalation-policies/${policyId}/rules/${ruleId}`);
  }
  ...
  async addIncidentResponder(
    incidentId: string,
    data: AddResponderRequest
  ): Promise<ResponderWithUser> {
    return this.request<ResponderWithUser>(`/api/v1/incidents/${incidentId}/responders`, {
      method: 'POST',
      body: JSON.stringify(data),
    });
  ...
  async linkAlertToIncident(
    incidentId: string,
    data: LinkAlertRequest
  ): Promise<IncidentAlertWithDetails> {
    return this.request<IncidentAlertWithDetails>(`/api/v1/incidents/${incidentId}/alerts`, {
      method: 'POST',
      body: JSON.stringify(data),
    });
  ...
  async createIncomingWebhookToken(
    data: CreateIncomingWebhookTokenRequest
  ): Promise<IncomingWebhookToken> {
    return this.request<IncomingWebhookToken>('/api/v1/webhooks/incoming', {
      method: 'POST',
      body: JSON.stringify(data),
    });
  ...
  async getDashboardMetrics(filter?: MetricsFilter): Promise<DashboardMetrics> {
    ...
    const queryString = params.toString();
    const endpoint = queryString
      ? `/api/v1/metrics/dashboard?${queryString}`
  ...
  async getAlertMetrics(filter?: MetricsFilter): Promise<AlertMetrics> {
    ...
    const queryString = params.toString();
    const endpoint = queryString
      ? `/api/v1/metrics/alerts?${queryString}`
  ...
  async getIncidentMetrics(filter?: MetricsFilter): Promise<IncidentMetrics> {
    ...
    const queryString = params.toString();
    const endpoint = queryString
      ? `/api/v1/metrics/incidents?${queryString}`
      : '/api/v1/metrics/incidents';
  ...
  async getNotificationMetrics(filter?: MetricsFilter): Promise<NotificationMetrics> {
    ...
    const queryString = params.toString();
    const endpoint = queryString
      ? `/api/v1/metrics/notifications?${queryString}`
  ...
  async getAlertTrend(filter?: MetricsFilter): Promise<AlertTrend> {
    ...
    const queryString = params.toString();
    const endpoint = queryString
      ? `/api/v1/metrics/alerts/trend?${queryString}`
