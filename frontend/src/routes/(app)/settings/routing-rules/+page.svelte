<script lang="ts">
  ...
  function editRule(rule: AlertRoutingRule) {
    ...
    conditions = parsedConditions.conditions?.length > 0
  ...
  async function handleSubmit() {
    ...
    if (actionTags.trim()) {
      actionsData.add_tags = actionTags.split(',').map(t => t.trim()).filter(t => t);
    }
  ...
  function formatActions(rule: AlertRoutingRule): string[] {
    ...
    if (a.assign_escalation_policy_id) actions.push(`Use policy: ${getPolicyName(a.assign_escalation_policy_id)}`);
    ...
    <div class="bg-white backdrop-blur-sm p-6 rounded-xl border border-primary-200 shadow-sm">
      ...
      <form on:submit|preventDefault={handleSubmit} class="space-y-6">
        ...
        <div class="flex gap-2">
          ...
          <Button type="button" variant="secondary" on:click={resetForm}>
            Cancel
          </Button>
        </div>
      </form>
