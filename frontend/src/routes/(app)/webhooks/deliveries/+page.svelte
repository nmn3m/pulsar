...
<div class="space-y-6">
	<div class="flex justify-between items-center">
		<div>
			...
			<p class="mt-2 text-gray-600">Monitor webhook delivery status and troubleshoot issues</p>
		</div>
	...
	{#if isLoading}
		<div class="text-center py-12">
			<p class="text-gray-500">Loading webhook deliveries...</p>
		</div>
	...
	{:else}
							...
							<div class="flex-1 min-w-0">
								...
								<div class="flex items-center gap-4 text-xs text-gray-500">
									...
									{#if delivery.next_retry_at}
										<span class="text-yellow-600">Next retry: {formatDate(delivery.next_retry_at)}</span>
									{/if}
								</div>
							</div>
						...
						<div class="border-t border-gray-200 px-6 py-4 bg-gray-50">
							<div class="space-y-4">
								...
								<div>
									...
									<pre class="bg-white p-3 rounded border border-gray-200 text-xs overflow-x-auto">{JSON.stringify(delivery.payload, null, 2)}</pre>
								</div>
								...
								{#if delivery.response_body}
									<div>
										...
										<pre class="bg-white p-3 rounded border border-gray-200 text-xs overflow-x-auto max-h-40">{delivery.response_body}</pre>
									</div>
								{/if}
								...
								{#if delivery.error_message}
									<div>
										<h4 class="text-sm font-medium text-red-700 mb-2">Error Message</h4>
								...
								<div class="grid grid-cols-2 gap-4 text-sm">
									<div>
										<span class="font-medium text-gray-700">Delivery ID:</span>
										<span class="text-gray-600 font-mono text-xs ml-2">{delivery.id}</span>
									</div>
									<div>
										<span class="font-medium text-gray-700">Endpoint ID:</span>
										<span class="text-gray-600 font-mono text-xs ml-2">{delivery.webhook_endpoint_id}</span>
									</div>
								</div>
							</div>
						</div>
		...
		<!-- Pagination -->
		<div class="flex items-center justify-between bg-white px-6 py-4 rounded-lg shadow">
