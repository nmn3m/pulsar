		...
		<div class="bg-white dark:bg-gray-800 p-6 rounded-lg shadow">
			...
			<div class="grid grid-cols-2 md:grid-cols-4 gap-4 text-sm">
				...
				<div>
					...
					<p class="font-medium text-gray-900 dark:text-white">
						{rotation.rotation_length} {rotation.rotation_type === 'daily' ? 'day(s)' : 'week(s)'}
					</p>
				</div>
				<div>
			...
			{#if showAddParticipantForm}
				<div class="mb-6 p-4 bg-gray-50 dark:bg-gray-700 rounded-lg">
					...
					<form on:submit|preventDefault={handleAddParticipant} class="space-y-3">
						<div>
							<label for="user-select" class="block text-sm font-medium text-gray-700 dark:text-gray-300 mb-1">
								Select User
							</label>
							<select
								id="user-select"
					...
					{#each participants as participant, index (participant.user_id)}
						<div class="flex items-center justify-between p-3 border border-gray-200 dark:border-gray-700 rounded-lg">
							<div class="flex items-center gap-3">
								<span class="w-6 h-6 flex items-center justify-center bg-primary-100 dark:bg-primary-900 text-primary-700 dark:text-primary-300 text-sm font-medium rounded-full">
									{index + 1}
								</span>
								<div>
									<p class="font-medium text-gray-900 dark:text-white">
