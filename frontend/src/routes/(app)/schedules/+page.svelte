...
<div class="space-y-6">
	<div class="flex justify-between items-center">
		<div>
			<h2 class="text-3xl font-bold text-gray-100">On-Call Schedules</h2>
			<p class="mt-2 text-gray-400">Manage on-call rotations and schedules</p>
		</div>
	...
	{#if showCreateForm}
		<div class="bg-space-800/50 backdrop-blur-sm p-6 rounded-xl border border-primary-500/30">
			<h3 class="text-lg font-semibold mb-4 text-gray-100">Create New Schedule</h3>
	...
	{#if $schedulesStore.isLoading}
		<div class="text-center py-12">
			...
			></div>
			<p class="mt-2 text-gray-400">Loading schedules...</p>
		</div>
	...
	{:else if $schedulesStore.schedules.length === 0}
		<div class="text-center py-12 bg-space-800/50 backdrop-blur-sm rounded-xl border border-space-600">
			<p class="text-gray-300">No schedules found</p>
	...
	{:else}
		<div class="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-6">
			{#each $schedulesStore.schedules as schedule (schedule.id)}
				<div class="bg-space-800/50 backdrop-blur-sm p-6 rounded-xl border border-space-600 hover:border-primary-500/30 transition-all duration-300 hover:shadow-lg hover:shadow-primary-500/10">
					<div class="flex justify-between items-start mb-4">
						<div class="flex-1">
							<h3 class="text-lg font-semibold text-gray-100">{schedule.name}</h3>
							{#if schedule.description}
								<p class="text-sm text-gray-400 mt-1">{schedule.description}</p>
							{/if}
