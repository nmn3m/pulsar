<script lang="ts">
	...
	async function handleAddTarget(ruleId: string, targetType: EscalationTargetType, targetId: string) {
		try {
	...
	{:else if policy}
		...
		<div class="bg-white p-6 rounded-lg shadow">
			...
			<div class="text-sm text-gray-600 space-y-1">
				<p>Repeat: {policy.repeat_enabled ? (policy.repeat_count ? `${policy.repeat_count} times` : 'Infinite') : 'Disabled'}</p>
			</div>
		</div>
