			...
			<div class="flex items-start justify-between mb-4">
				...
				<div class="flex gap-2">
					{#if alert.status === 'open'}
						<Button variant="primary" size="sm" on:click={handleAcknowledge}>
							Acknowledge
						</Button>
					{/if}
					{#if alert.status !== 'closed'}
						<Button variant="secondary" size="sm" on:click={() => (showSnoozeForm = !showSnoozeForm)}>
							Snooze
						</Button>
			...
			<div class="grid grid-cols-2 gap-4 pt-4 border-t border-gray-200">
				...
				{#if alert.acknowledged_at}
					<div>
						...
						<div class="font-medium">{dayjs(alert.acknowledged_at).format('MMM D, YYYY h:mm A')}</div>
