...
// @Produce      json
// @Security     BearerAuth
// @Param        request body usecase.CreateAPIKeyRequest true "Create API key request"
...
func (h *APIKeyHandler) Create(c *gin.Context) {
	...
	response, err := h.apiKeyUsecase.CreateAPIKey(c.Request.Context(), orgID, userID, &req)
	if err != nil {
		c.JSON(http.StatusBadRequest, gin.H{"error": err.Error()})
		return
	}
...
func (h *APIKeyHandler) List(c *gin.Context) {
	...
	keys, err := h.apiKeyUsecase.ListUserAPIKeys(c.Request.Context(), userID)
	if err != nil {
...
func (h *APIKeyHandler) Get(c *gin.Context) {
	...
	key, err := h.apiKeyUsecase.GetAPIKey(c.Request.Context(), id)
	if err != nil {
...
// @Security     BearerAuth
// @Param        id path string true "API Key ID" format(uuid)
// @Param        request body usecase.UpdateAPIKeyRequest true "Update API key request"
// @Success      200 {object} domain.APIKey
// @Failure      400 {object} map[string]string
// @Failure      404 {object} map[string]string
...
func (h *APIKeyHandler) Update(c *gin.Context) {
	...
	existingKey, err := h.apiKeyUsecase.GetAPIKey(c.Request.Context(), id)
	if err != nil {
		c.JSON(http.StatusNotFound, gin.H{"error": "API key not found"})
		return
	}
	...
	var req usecase.UpdateAPIKeyRequest
	if err := c.ShouldBindJSON(&req); err != nil {
		c.JSON(http.StatusBadRequest, gin.H{"error": err.Error()})
		return
	}

	key, err := h.apiKeyUsecase.UpdateAPIKey(c.Request.Context(), id, &req)
	if err != nil {
		c.JSON(http.StatusBadRequest, gin.H{"error": err.Error()})
		return
	}
...
func (h *APIKeyHandler) Revoke(c *gin.Context) {
	...
	if err := h.apiKeyUsecase.RevokeAPIKey(c.Request.Context(), id); err != nil {
		c.JSON(http.StatusBadRequest, gin.H{"error": err.Error()})
		return
	}
