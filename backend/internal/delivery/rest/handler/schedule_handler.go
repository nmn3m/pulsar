...
func (h *ScheduleHandler) List(c *gin.Context) {
	...
	schedules, err := h.scheduleUsecase.ListSchedules(c.Request.Context(), orgID, page, pageSize)
	if err != nil {
...
// @Produce      json
// @Security     BearerAuth
// @Param        request  body      usecase.CreateScheduleRequest  true  "Schedule creation request"
...
func (h *ScheduleHandler) Create(c *gin.Context) {
	...
	var req usecase.CreateScheduleRequest
	if err := c.ShouldBindJSON(&req); err != nil {
		c.JSON(http.StatusBadRequest, gin.H{"error": err.Error()})
		return
	}

	schedule, err := h.scheduleUsecase.CreateSchedule(c.Request.Context(), orgID, &req)
	if err != nil {
		c.JSON(http.StatusBadRequest, gin.H{"error": err.Error()})
		return
	}
...
func (h *ScheduleHandler) Get(c *gin.Context) {
	...
	schedule, err := h.scheduleUsecase.GetScheduleWithRotations(c.Request.Context(), id)
	if err != nil {
...
// @Security     BearerAuth
// @Param        id       path      string                          true  "Schedule ID"  format(uuid)
// @Param        request  body      usecase.UpdateScheduleRequest   true  "Schedule update request"
...
func (h *ScheduleHandler) Update(c *gin.Context) {
	...
	var req usecase.UpdateScheduleRequest
	if err := c.ShouldBindJSON(&req); err != nil {
		c.JSON(http.StatusBadRequest, gin.H{"error": err.Error()})
		return
	}

	schedule, err := h.scheduleUsecase.UpdateSchedule(c.Request.Context(), id, &req)
	if err != nil {
		c.JSON(http.StatusBadRequest, gin.H{"error": err.Error()})
		return
	}
...
func (h *ScheduleHandler) Delete(c *gin.Context) {
	...
	if err := h.scheduleUsecase.DeleteSchedule(c.Request.Context(), id); err != nil {
		c.JSON(http.StatusInternalServerError, gin.H{"error": err.Error()})
		return
	}
...
func (h *ScheduleHandler) ListRotations(c *gin.Context) {
	...
	rotations, err := h.scheduleUsecase.ListRotations(c.Request.Context(), scheduleID)
	if err != nil {
		c.JSON(http.StatusInternalServerError, gin.H{"error": err.Error()})
		return
	}
...
// @Security     BearerAuth
// @Param        id       path      string                          true  "Schedule ID"  format(uuid)
// @Param        request  body      usecase.CreateRotationRequest   true  "Rotation creation request"
...
func (h *ScheduleHandler) CreateRotation(c *gin.Context) {
	...
	var req usecase.CreateRotationRequest
	if err := c.ShouldBindJSON(&req); err != nil {
		c.JSON(http.StatusBadRequest, gin.H{"error": err.Error()})
		return
	}

	rotation, err := h.scheduleUsecase.CreateRotation(c.Request.Context(), scheduleID, &req)
	if err != nil {
		c.JSON(http.StatusBadRequest, gin.H{"error": err.Error()})
		return
	}
...
func (h *ScheduleHandler) GetRotation(c *gin.Context) {
	...
	rotation, err := h.scheduleUsecase.GetRotation(c.Request.Context(), rotationID)
	if err != nil {
		c.JSON(http.StatusNotFound, gin.H{"error": err.Error()})
		return
	}
...
// @Param        id          path      string                          true  "Schedule ID"   format(uuid)
// @Param        rotationId  path      string                          true  "Rotation ID"   format(uuid)
// @Param        request     body      usecase.UpdateRotationRequest   true  "Rotation update request"
// @Success      200         {object}  domain.ScheduleRotation
// @Failure      400         {object}  map[string]string
...
func (h *ScheduleHandler) UpdateRotation(c *gin.Context) {
	...
	var req usecase.UpdateRotationRequest
	if err := c.ShouldBindJSON(&req); err != nil {
		c.JSON(http.StatusBadRequest, gin.H{"error": err.Error()})
		return
	}

	rotation, err := h.scheduleUsecase.UpdateRotation(c.Request.Context(), rotationID, &req)
	if err != nil {
		c.JSON(http.StatusBadRequest, gin.H{"error": err.Error()})
		return
	}
...
func (h *ScheduleHandler) DeleteRotation(c *gin.Context) {
	...
	if err := h.scheduleUsecase.DeleteRotation(c.Request.Context(), rotationID); err != nil {
		c.JSON(http.StatusInternalServerError, gin.H{"error": err.Error()})
		return
	}
...
func (h *ScheduleHandler) ListParticipants(c *gin.Context) {
	...
	participants, err := h.scheduleUsecase.ListParticipants(c.Request.Context(), rotationID)
	if err != nil {
		c.JSON(http.StatusInternalServerError, gin.H{"error": err.Error()})
		return
	}
...
// @Param        id          path      string                          true  "Schedule ID"   format(uuid)
// @Param        rotationId  path      string                          true  "Rotation ID"   format(uuid)
// @Param        request     body      usecase.AddParticipantRequest   true  "Add participant request"
...
func (h *ScheduleHandler) AddParticipant(c *gin.Context) {
	...
	participant, err := h.scheduleUsecase.AddParticipant(c.Request.Context(), rotationID, &req)
	if err != nil {
		c.JSON(http.StatusBadRequest, gin.H{"error": err.Error()})
		return
	}
...
func (h *ScheduleHandler) RemoveParticipant(c *gin.Context) {
	...
	if err := h.scheduleUsecase.RemoveParticipant(c.Request.Context(), rotationID, userID); err != nil {
		c.JSON(http.StatusInternalServerError, gin.H{"error": err.Error()})
		return
	}
...
// @Param        id          path      string                              true  "Schedule ID"   format(uuid)
// @Param        rotationId  path      string                              true  "Rotation ID"   format(uuid)
// @Param        request     body      usecase.ReorderParticipantsRequest  true  "Reorder participants request"
// @Success      200         {object}  map[string]string
// @Failure      400         {object}  map[string]string
...
func (h *ScheduleHandler) ReorderParticipants(c *gin.Context) {
	...
	var req usecase.ReorderParticipantsRequest
	if err := c.ShouldBindJSON(&req); err != nil {
		c.JSON(http.StatusBadRequest, gin.H{"error": err.Error()})
		return
	}

	if err := h.scheduleUsecase.ReorderParticipants(c.Request.Context(), rotationID, &req); err != nil {
		c.JSON(http.StatusBadRequest, gin.H{"error": err.Error()})
		return
	}
...
func (h *ScheduleHandler) ListOverrides(c *gin.Context) {
	...
	overrides, err := h.scheduleUsecase.ListOverrides(c.Request.Context(), scheduleID, start, end)
	if err != nil {
		c.JSON(http.StatusInternalServerError, gin.H{"error": err.Error()})
		return
	}
...
// @Security     BearerAuth
// @Param        id       path      string                          true  "Schedule ID"  format(uuid)
// @Param        request  body      usecase.CreateOverrideRequest   true  "Override creation request"
...
func (h *ScheduleHandler) CreateOverride(c *gin.Context) {
	...
	var req usecase.CreateOverrideRequest
	if err := c.ShouldBindJSON(&req); err != nil {
		c.JSON(http.StatusBadRequest, gin.H{"error": err.Error()})
		return
	}

	override, err := h.scheduleUsecase.CreateOverride(c.Request.Context(), scheduleID, &req)
	if err != nil {
		c.JSON(http.StatusBadRequest, gin.H{"error": err.Error()})
		return
	}
...
func (h *ScheduleHandler) GetOverride(c *gin.Context) {
	...
	override, err := h.scheduleUsecase.GetOverride(c.Request.Context(), overrideID)
	if err != nil {
		c.JSON(http.StatusNotFound, gin.H{"error": err.Error()})
		return
	}
...
// @Param        id          path      string                          true  "Schedule ID"   format(uuid)
// @Param        overrideId  path      string                          true  "Override ID"   format(uuid)
// @Param        request     body      usecase.UpdateOverrideRequest   true  "Override update request"
// @Success      200         {object}  domain.ScheduleOverride
// @Failure      400         {object}  map[string]string
...
func (h *ScheduleHandler) UpdateOverride(c *gin.Context) {
	...
	var req usecase.UpdateOverrideRequest
	if err := c.ShouldBindJSON(&req); err != nil {
		c.JSON(http.StatusBadRequest, gin.H{"error": err.Error()})
		return
	}

	override, err := h.scheduleUsecase.UpdateOverride(c.Request.Context(), overrideID, &req)
	if err != nil {
		c.JSON(http.StatusBadRequest, gin.H{"error": err.Error()})
		return
	}
...
func (h *ScheduleHandler) DeleteOverride(c *gin.Context) {
	...
	if err := h.scheduleUsecase.DeleteOverride(c.Request.Context(), overrideID); err != nil {
		c.JSON(http.StatusInternalServerError, gin.H{"error": err.Error()})
		return
	}
...
func (h *ScheduleHandler) GetOnCall(c *gin.Context) {
	...
	onCallUser, err := h.scheduleUsecase.GetOnCallUser(c.Request.Context(), scheduleID, at)
	if err != nil {
		c.JSON(http.StatusNotFound, gin.H{"error": err.Error()})
		return
	}
