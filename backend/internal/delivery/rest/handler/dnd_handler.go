...
type DNDHandler struct {
	dndUsecase *usecase.DNDUsecase
}

func NewDNDHandler(dndUsecase *usecase.DNDUsecase) *DNDHandler {
	return &DNDHandler{dndUsecase: dndUsecase}
}
...
func (h *DNDHandler) GetDNDSettings(c *gin.Context) {
	...
	settings, err := h.dndUsecase.GetSettings(c.Request.Context(), userID.(uuid.UUID))
	if err != nil {
...
// @Accept json
// @Produce json
// @Param request body usecase.UpdateDNDSettingsRequest true "DND settings"
// @Success 200 {object} domain.UserDNDSettings
...
func (h *DNDHandler) UpdateDNDSettings(c *gin.Context) {
	...
	var req usecase.UpdateDNDSettingsRequest
	if err := c.ShouldBindJSON(&req); err != nil {
		c.JSON(http.StatusBadRequest, gin.H{"error": err.Error()})
		return
	}

	settings, err := h.dndUsecase.UpdateSettings(c.Request.Context(), userID.(uuid.UUID), &req)
	if err != nil {
		c.JSON(http.StatusInternalServerError, gin.H{"error": err.Error()})
		return
	}
...
// @Accept json
// @Produce json
// @Param request body usecase.AddDNDOverrideRequest true "Override details"
// @Success 200 {object} domain.UserDNDSettings
// @Failure 400 {object} map[string]string
// @Failure 401 {object} map[string]string
// @Failure 500 {object} map[string]string
...
func (h *DNDHandler) AddDNDOverride(c *gin.Context) {
	...
	var req usecase.AddDNDOverrideRequest
	if err := c.ShouldBindJSON(&req); err != nil {
		c.JSON(http.StatusBadRequest, gin.H{"error": err.Error()})
		return
	}
	...
	settings, err := h.dndUsecase.AddOverride(c.Request.Context(), userID.(uuid.UUID), &req)
	if err != nil {
		c.JSON(http.StatusInternalServerError, gin.H{"error": err.Error()})
		return
	}
...
func (h *DNDHandler) RemoveDNDOverride(c *gin.Context) {
	...
	settings, err := h.dndUsecase.RemoveOverride(c.Request.Context(), userID.(uuid.UUID), index)
	if err != nil {
		c.JSON(http.StatusInternalServerError, gin.H{"error": err.Error()})
		return
	}
...
func (h *DNDHandler) DeleteDNDSettings(c *gin.Context) {
	...
	err := h.dndUsecase.DeleteSettings(c.Request.Context(), userID.(uuid.UUID))
	if err != nil {
		c.JSON(http.StatusInternalServerError, gin.H{"error": err.Error()})
		return
	}
...
func (h *DNDHandler) CheckDNDStatus(c *gin.Context) {
	...
	inDND, err := h.dndUsecase.IsInDNDMode(c.Request.Context(), userID.(uuid.UUID), priority)
	if err != nil {
		c.JSON(http.StatusInternalServerError, gin.H{"error": err.Error()})
		return
	}
