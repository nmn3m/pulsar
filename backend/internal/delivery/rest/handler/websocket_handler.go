...
func (h *WebSocketHandler) HandleWebSocket(c *gin.Context) {
	...
	h.wsUsecase.GetHub().Register <- client
...
func (h *WebSocketHandler) readPump(conn *websocket.Conn, client *domain.WSClient) {
	defer func() {
		h.wsUsecase.GetHub().Unregister <- client
		conn.Close()
	}()
...
func (h *WebSocketHandler) GetStats(c *gin.Context) {
	...
	stats := gin.H{
		"organization_client_count": h.wsUsecase.GetClientCount(orgID),
		"total_client_count":        h.wsUsecase.GetTotalClientCount(),
	}
