...
func NewIncomingWebhookHandler(webhookUsecase *usecase.WebhookUsecase, alertUsecase *usecase.AlertUsecase, logger *zap.Logger) *IncomingWebhookHandler {
...
func (h *IncomingWebhookHandler) ReceiveWebhook(c *gin.Context) {
	...
	webhookToken, err := h.webhookUsecase.GetIncomingTokenByToken(c.Request.Context(), token)
	if err != nil {
	...
	// Update usage stats
	if err := h.webhookUsecase.UpdateIncomingTokenUsage(c.Request.Context(), webhookToken.ID); err != nil {
	...
	var alerts []*usecase.CreateAlertRequest
	...
	for _, alertReq := range alerts {
		...
		// Create alert
		alert, err := h.alertUsecase.CreateAlert(c.Request.Context(), webhookToken.OrganizationID, alertReq)
		if err != nil {
...
func (h *IncomingWebhookHandler) parsePrometheusWebhook(body []byte) ([]*usecase.CreateAlertRequest, error) {
	var payload struct {
		Alerts []struct {
	...
	for _, prometheusAlert := range payload.Alerts {
		...
		alerts = append(alerts, &usecase.CreateAlertRequest{
			Source:      "prometheus",
			Priority:    priority,
			Message:     message,
...
func (h *IncomingWebhookHandler) parseGrafanaWebhook(body []byte) ([]*usecase.CreateAlertRequest, error) {
	var payload struct {
	...
	// Skip resolved alerts
	if payload.State == "ok" {
		return []*usecase.CreateAlertRequest{}, nil
	}
	...
	return []*usecase.CreateAlertRequest{
		{
			Source:      "grafana",
			Priority:    priority,
			Message:     message,
...
func (h *IncomingWebhookHandler) parseGenericWebhook(body []byte) ([]*usecase.CreateAlertRequest, error) {
	var payload struct {
		Message     string   `json:"message"`
