...
func NewRoutingHandler(routingUsecase *usecase.RoutingUsecase) *RoutingHandler {
...
func (h *RoutingHandler) List(c *gin.Context) {
	...
	rules, err := h.routingUsecase.ListRules(c.Request.Context(), orgID, page, pageSize)
	if err != nil {
...
// @Produce      json
// @Security     BearerAuth
// @Param        request  body      usecase.CreateRoutingRuleRequest  true  "Routing rule creation request"
...
func (h *RoutingHandler) Create(c *gin.Context) {
	...
	rule, err := h.routingUsecase.CreateRule(c.Request.Context(), orgID, &req)
	if err != nil {
		c.JSON(http.StatusBadRequest, gin.H{"error": err.Error()})
		return
	}
...
func (h *RoutingHandler) Get(c *gin.Context) {
	...
	rule, err := h.routingUsecase.GetRule(c.Request.Context(), id)
	if err != nil {
...
// @Security     BearerAuth
// @Param        id       path      string                           true  "Routing rule ID"  format(uuid)
// @Param        request  body      usecase.UpdateRoutingRuleRequest  true  "Routing rule update request"
...
func (h *RoutingHandler) Update(c *gin.Context) {
	...
	rule, err := h.routingUsecase.UpdateRule(c.Request.Context(), id, &req)
	if err != nil {
		c.JSON(http.StatusBadRequest, gin.H{"error": err.Error()})
		return
	}
...
func (h *RoutingHandler) Delete(c *gin.Context) {
	...
	if err := h.routingUsecase.DeleteRule(c.Request.Context(), id); err != nil {
		c.JSON(http.StatusInternalServerError, gin.H{"error": err.Error()})
		return
	}
...
// @Produce      json
// @Security     BearerAuth
// @Param        request  body      usecase.ReorderRoutingRulesRequest  true  "Routing rules reorder request"
...
func (h *RoutingHandler) Reorder(c *gin.Context) {
	...
	var req usecase.ReorderRoutingRulesRequest
	if err := c.ShouldBindJSON(&req); err != nil {
		c.JSON(http.StatusBadRequest, gin.H{"error": err.Error()})
		return
	}

	if err := h.routingUsecase.ReorderRules(c.Request.Context(), orgID, &req); err != nil {
		c.JSON(http.StatusInternalServerError, gin.H{"error": err.Error()})
		return
	}
