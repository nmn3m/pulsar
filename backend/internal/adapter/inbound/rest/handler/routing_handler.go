...
func NewRoutingHandler(routingService inbound.RoutingService) *RoutingHandler {
...
// @Produce      json
// @Security     BearerAuth
// @Param        request  body      dto.CreateRoutingRuleRequest  true  "Routing rule creation request"
...
// @Security     BearerAuth
// @Param        id       path      string                           true  "Routing rule ID"  format(uuid)
// @Param        request  body      dto.UpdateRoutingRuleRequest  true  "Routing rule update request"
...
// @Produce      json
// @Security     BearerAuth
// @Param        request  body      dto.ReorderRoutingRulesRequest  true  "Routing rules reorder request"
...
func (h *RoutingHandler) Reorder(c *gin.Context) {
	...
	var req dto.ReorderRoutingRulesRequest
	if err := c.ShouldBindJSON(&req); err != nil {
		c.JSON(http.StatusBadRequest, gin.H{"error": err.Error()})
		return
	}
