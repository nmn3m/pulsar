...
// WebSocketUsecase manages WebSocket connections and message broadcasting
...
// NewWebSocketUsecase creates a new WebSocket usecase
func NewWebSocketUsecase(logger *zap.Logger) *WebSocketUsecase {
...
func (s *WebSocketUsecase) GetHub() *domain.WSHub {
	return s.hub
}

// Run starts the WebSocket hub
func (s *WebSocketUsecase) Run() {
	for {
		select {
...
func (s *WebSocketUsecase) registerClient(client *domain.WSClient) {
	s.mu.Lock()
	defer s.mu.Unlock()
...
func (s *WebSocketUsecase) unregisterClient(client *domain.WSClient) {
	s.mu.Lock()
	defer s.mu.Unlock()
...
func (s *WebSocketUsecase) broadcastMessage(message *domain.WSMessage) {
	s.mu.RLock()
	defer s.mu.RUnlock()
...
func (s *WebSocketUsecase) BroadcastAlertEvent(eventType domain.WSEventType, orgID uuid.UUID, alert *domain.Alert) {
...
func (s *WebSocketUsecase) BroadcastIncidentEvent(eventType domain.WSEventType, orgID uuid.UUID, incident *domain.Incident) {
	payload := map[string]interface{}{
...
func (s *WebSocketUsecase) BroadcastIncidentTimelineEvent(orgID, incidentID uuid.UUID, event *domain.IncidentTimelineEvent) {
	payload := map[string]interface{}{
...
func (s *WebSocketUsecase) GetClientCount(orgID uuid.UUID) int {
	s.mu.RLock()
	defer s.mu.RUnlock()
...
func (s *WebSocketUsecase) GetTotalClientCount() int {
	s.mu.RLock()
	defer s.mu.RUnlock()
