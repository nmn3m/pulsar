...
type TeamService struct {
	teamRepo       outbound.TeamRepository
	userRepo       outbound.UserRepository
	invitationRepo outbound.TeamInvitationRepository
...
func NewTeamService(teamRepo outbound.TeamRepository, userRepo outbound.UserRepository) *TeamService {
	return &TeamService{
		teamRepo: teamRepo,
		userRepo: userRepo,
	}
...
func (s *TeamService) SetInvitationRepo(repo outbound.TeamInvitationRepository) {
...
func (s *TeamService) CreateTeam(ctx context.Context, orgID uuid.UUID, req *dto.CreateTeamRequest) (*domain.Team, error) {
	team := &domain.Team{
		ID:             uuid.New(),
...
func (s *TeamService) GetTeamWithMembers(ctx context.Context, id uuid.UUID) (*dto.TeamWithMembers, error) {
	team, err := s.teamRepo.GetByID(ctx, id)
	if err != nil {
		return nil, fmt.Errorf("failed to get team: %w", err)
	}
...
func (s *TeamService) UpdateTeam(ctx context.Context, id uuid.UUID, req *dto.UpdateTeamRequest) (*domain.Team, error) {
	team, err := s.teamRepo.GetByID(ctx, id)
	if err != nil {
		return nil, fmt.Errorf("failed to get team: %w", err)
	}
...
func (s *TeamService) AddMember(ctx context.Context, teamID uuid.UUID, req *dto.AddTeamMemberRequest) error {
	// Parse role
...
func (s *TeamService) AddMemberOrInvite(ctx context.Context, teamID, orgID, inviterID uuid.UUID, req *dto.InviteMemberRequest) (*dto.InvitationResponse, error) {
	// Parse role
	role := domain.TeamRoleMember
	if req.Role != "" {
		role = domain.TeamRole(req.Role)
...
func (s *TeamService) UpdateMemberRole(ctx context.Context, teamID, userID uuid.UUID, req *dto.UpdateTeamMemberRoleRequest) error {
	role := domain.TeamRole(req.Role)
